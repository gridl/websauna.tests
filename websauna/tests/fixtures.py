...
from websauna.tests.utils import get_qual_name
...
@pytest.fixture(scope='session')
def paster_config(request: FixtureRequest, test_config_path, ini_settings) -> tuple:
...
@pytest.fixture(scope='session')
def app(request: FixtureRequest, ini_settings: dict, **settings_overrides) -> Router:
...
@pytest.fixture(scope='session')
def init(request: FixtureRequest, app):
...
@pytest.fixture(scope='session')
def registry(request: FixtureRequest, app) -> Registry:
...
def create_test_dbsession(request: FixtureRequest, registry: Registry, transaction_manager=transaction.manager) -> Session:
...
@pytest.fixture()
def dbsession(request: FixtureRequest, app) -> Session:
...
@pytest.fixture()
def http_request(request: FixtureRequest):
...
@pytest.fixture()
def test_request(request: FixtureRequest, dbsession, registry) -> IRequest:
